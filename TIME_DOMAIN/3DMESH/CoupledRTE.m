...
% SORTIES 
%
function [TAU] = CoupledRTE(Tini,C0,P,W,Xsrc,Ysrc,Tobs,X1,X2,X3,eps);
   %
   ...
   while (abs(fonction)>eps)
      ...
      %
      if (TAU_PP > TAU_P)
         nn_bounds=nn_bounds+1;
         if (nn_bounds > 20)
            disp('Calling Bisection')
            [TAU_PP] = bisection(P,W,Tobs,R,Psi,X1,X2,X3,C0,eps);
            fonction=0;
         end
      end
      % 
