...
%     count = 0;
   for j = 1:nt
      jadv = floor( (tobs_real(i,j)) / (1*dt) );
      w    =        (tobs_real(i,j)) / (1*dt)    - jadv;
      %fprintf(' jadv = %i \n', jadv);
      %fprintf(' w = %g \n', w);
      %pause
      %toto(i,j) = jadv;%*dt;
...
pobs_discretM=zeros(ns,ns*nt);
pac_bisM=zeros(ntadv,1);
for j = 1:nt-1
    for i = 1:ns
        lM = j + 1 + floor(robs_real(i,j)/(c0*dt));
        wM = 1 - (robs_real(i,j)/(c0*dt) - floor(robs_real(i,j)/(c0*dt)));
        pinter = (pobs_real(i,j+1)-pobs_real(i,j)) * wM + pobs_real(i,j);
        pobs_discretM(i,lM) = pinter;
    end
end

for lM = floor(1 + rmax/(c0*dt)):floor(nt + rmin/(c0*dt))
    for i = 1:ns
        pac_bisM(lM) = pac_bisM(lM) + pobs_discretM(i,lM);
    end
end
