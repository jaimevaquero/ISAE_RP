...
if nth == 1:
    ...
    dtho = Ltho / (nth - 1)
    tho[0] = 0.
    for io in range(1,nth):
        tho[io] = tho[io-1] + dtho
        
xo = np.zeros([nr,nth])
yo = np.zeros([nr,nth])
zo = 0.

xo = np.einsum('i,j->ij',ro,np.cos(tho))
yo = np.einsum('i,j->ij',ro,np.sin(tho))
...
Lhz = 7.5*27.3*0.019 # It is half the extrusion length! Extrusion is symmetric
ScaleNzs = 10.
...
pacObs  = np.zeros([nr,nth,nt])
timeObs = np.zeros([nr,nth,nt])
prms    = np.zeros([nr,nth])
#kmin    = np.zeros([nr,nth])
#kmax    = np.zeros([nr,nth])

for jo in range(0,nth):
    print('  Farassat Loop    : ' + str(jo+1) + ' of ' + str(nth))
    for io in range(0,nr):
        pobs,tobs,robs = Formulation1A(nt,nxsFWH,nzs,t,xsExt,ysExt,zsExt,normalExt,elesurfExt,Aext_ux,Aext_uy, \
                  Aext_uz,Aext_p,Aext_rho,np.array([xo[io,jo],yo[io,jo],zo]),dt,c0,p0,rho0)
#print('')                
...
#                wij[j,l,kadv] = 0.0
        ...
        for k in range(0,nlM):
            ...
            if k < pacObs.shape[2]:
                pacObs[io,jo,k]  = pac[k]
                timeObs[io,jo,k] = tadv[k]
            else:
                pacObs  = np.append(pacObs ,np.zeros([nr,nth,nlM-nt]),axis=2)
                timeObs = np.append(timeObs,np.zeros([nr,nth,nlM-nt]),axis=2)
                pacObs[io,jo,k]  = pac[k]
                timeObs[io,jo,k] = tadv[k]
        ...
        prms[io,jo] = np.sqrt(                                                                  \
                          np.mean(                                                              \
                              (pacObs[io,jo,kmin:kmax-1]-np.mean(pacObs[io,jo,kmin:kmax-1]))**2.\
                              )
                         )
...
#plt.plot(np.linspace(0,36*0.35789,len(pacFromCyl))+35,pacFromCyl,'k-')
#plt.xlabel('tU0/D')
#plt.ylabel('pac')
#plt.grid()
#plt.show()

fileChXBONPprms75DX = np.loadtxt(open('../../CYLINDRE_W/FWH/prmsAdim75DX_CharlesX_BonP_2w_6p.dat', 'rb'),delimiter = ',', skiprows = 1)
fileChXBONPprms75DY = np.loadtxt(open('../../CYLINDRE_W/FWH/prmsAdim75DY_CharlesX_BonP_2w_6p.dat', 'rb'),delimiter = ',', skiprows = 1)

plt.figure()
plt.plot(prms[-1,:]*np.cos(tho)/(rho0*c0**2),prms[-1,:]*np.sin(tho)/(rho0*c0**2),'k--')
plt.plot(fileChXBONPprms75DX,fileChXBONPprms75DY,'k-')
plt.grid()
plt.show()

plt.figure()
plt.plot(np.fft.rfftfreq(kmax-kmin,dt),20.*np.log10(abs(np.fft.rfft(pacObs[-1,-1,kmin:kmax-1]))),'k-o')
plt.plot(np.fft.rfftfreq(kmax-kmin,dt),20.*np.log10(abs(np.fft.rfft(pacObs[-1,0,kmin:kmax-1]))),'r-x')
plt.grid()
plt.show()
